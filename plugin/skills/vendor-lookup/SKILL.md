...
The canonical vendor list lives in `order_processing/Vendor Information.md` (and the machine-friendly `Vendor Information.jsonl`). Start there:

1. **Exact-name match.** Look for a folder in `order_processing/` whose name matches the vendor the user mentioned, case-insensitive. Slashes in the master list become ` - ` in folder names — `Assa Abloy/Pemco/Rockwood` is the folder `Assa Abloy - Pemco - Rockwood`.
2. **Sub-brand / alias match.** If no folder matches, scan `Vendor Information.md` — many rows combine multiple brands. Common sub-brand → parent mappings:
   - Pemco, Rockwood, Pemko → `Assa Abloy - Pemco - Rockwood/`
   - US Lock, HD Supply → `Home Depot - US lock - HD Supply/`
   - Emery Jensen → `ACE - Emery Jensen/`
   - Sentry → `Prime-Line - Sentry/`
   - Portals → `Bohle - Portals/`
   Also check each vendor's `Aliases / sub-brands` section inside `<Vendor> - Vendor Info.md`.
...
|---|---|
| Contact / phone / email / who to reach | `<Vendor>/<Vendor> - Vendor Info.md` |
| Can we dropship? Ships under our account? Expedited? | `<Vendor>/<Vendor> - Vendor Info.md` |
| When is a PO submitted? When do we receive shipments? | `<Vendor>/<Vendor> - Vendor Info.md` |
| DK point of contact (Elina, Yana, Kirill, etc.) for this vendor | `<Vendor>/<Vendor> - Vendor Info.md` |
| How to place / track / escalate an order | `<Vendor>/<Vendor> Process Document.md` |
| Damage, missing item, wrong item, defect, return, RMA | `<Vendor>/<Vendor> - Issue Resolution Notes.md` |
...
1. Resolve: exact match, folder `Deltana/`.
2. Pick file: contact question → `Deltana/Deltana - Vendor Info.md`.
...
1. Resolve: "Pemco" is not a folder; it's a sub-brand of `Assa Abloy - Pemco - Rockwood`.
2. Pick file: shipping question → `Assa Abloy - Pemco - Rockwood/Assa Abloy - Pemco - Rockwood - Vendor Info.md`.
3. Read the file.
4. Answer:
...
>
> Sources:
> - Assa Abloy - Pemco - Rockwood/Assa Abloy - Pemco - Rockwood - Vendor Info.md

**Example 3 — ambiguous vendor, must ask**

User: "What's the phone for Glass?"

1. Resolve: "Glass" alone is not a match. Scan the master list — could be `Glass Warehouse`, `STRUCTURE GLASS SOLUTIONS`, or possibly a sub-brand.
...
1. Resolve: folder `Abaco Machines/` exists.
2. Pick file: process question → `Abaco Machines/Abaco Machines Process Document.md`.
...
>
> Sources:
> - Abaco Machines/Abaco Machines - Vendor Info.md
> - Abaco Machines/Abaco Machines Process Document.md (stub)
> - _shared_sops/SOP.md
